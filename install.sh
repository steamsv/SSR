...
if [[ -e ${libsodiumfilea} ]];then
    ...
    wget -q https://codeload.github.com/steamsv/libsodium/tar.gz/1.0.17
...
cd /usr/local
git clone https://github.com/steamsv/shadowsocksr
cd ./shadowsocksr
...
if [[ -d /usr/local/SSR-Bash-Python ]];then
    if [[ $yn == [yY] ]];then
        rm -rf /usr/local/SSR-Bash-Python
        cd /usr/local
        git clone https://github.com/steamsv/SSR-Bash-Python.git
    fi
...
if [[ $1 == "develop" ]];then
	wget -q -N --no-check-certificate -O /usr/local/bin/ssr https://raw.githubusercontent.com/steamsv/SSR-Bash-Python/master/ssr
